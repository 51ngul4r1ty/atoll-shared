...
const camelCase = (val: string): string => {
    if (!val) {
        return val;
    }
...
const getIsUnsubscribed = () => adapterState.subscriptionStatus === TAdapterSubscriptionStatus.Unsubscribed;

const normalizeFlag = (flagName: TFlagName, flagValue?: TFlagVariation): TFlag => [
    camelCase(flagName),
    // Multi variate flags contain a string or `null` - `false` seems more natural.
    flagValue === null || flagValue === undefined ? false : flagValue
];
export const normalizeFlags = (rawFlags: TFlags) =>
    Object.entries(rawFlags).reduce<TFlags>((normalizedFlags: TFlags, [flagName, flagValue]) => {
        const [normalizedFlagName, normalizedFlagValue]: TFlag = normalizeFlag(flagName, flagValue);
...
export const updateFlags = (flags: TFlags) => {
    const isAdapterConfigured = adapterState.configurationStatus === TAdapterConfigurationStatus.Configured;
...
class MemoryAdapter implements TMemoryAdapterInterface {
    ...
    configure(adapterArgs: TMemoryAdapterArgs, adapterEventHandlers: TAdapterEventHandlers) {
    ...
    reconfigure(adapterArgs: TMemoryAdapterArgs, _adapterEventHandlers: TAdapterEventHandlers) {
        adapterState.configurationStatus = TAdapterConfigurationStatus.Configuring;
    ...
    waitUntilConfigured() {
        return new Promise((resolve) => {
            if (adapterState.configurationStatus === TAdapterConfigurationStatus.Configured) resolve();
    ...
    getIsReady() {
        ...
        return this.getIsConfigurationStatus(TAdapterConfigurationStatus.Configured);
    }
}
